...
namespace TpacTool.IO
{
    ...
    internal static class Decoder
    {
		public struct DecodeResult
		{
			...
			public DecodeResult(byte bytesPerPixel, byte samplesPerPixel)
			{
				this.bytesPerPixel = bytesPerPixel;
				this.samplesPerPixel = samplesPerPixel;
			}
		}
        public static DecodeResult GetBytesAndSamplesPerPixel(ImageHeader header)
        {
            var bitDepthCorrected = (header.BitDepth + 7) / 8;

            var samplesPerPixel = SamplesPerPixel(header);

            return new DecodeResult((byte)(samplesPerPixel * bitDepthCorrected), samplesPerPixel);
        }
