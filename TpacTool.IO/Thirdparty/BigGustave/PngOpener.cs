    ...
    internal static class PngOpener
    {
        public static Png Open(Stream stream, IChunkVisitor chunkVisitor = null)
        {
            ...
            using (var output = new MemoryStream())
            {
                ...
                var decodeResult = Decoder.GetBytesAndSamplesPerPixel(imageHeader);

                bytesOut = Decoder.Decode(bytesOut, imageHeader, decodeResult.bytesPerPixel, decodeResult.samplesPerPixel);

                return new Png(imageHeader, new RawPngData(bytesOut, decodeResult.bytesPerPixel, imageHeader.Width, imageHeader.InterlaceMethod, palette, imageHeader.ColorType));
            }
        }
